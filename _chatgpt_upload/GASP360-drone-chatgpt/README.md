...
## Running brush to reconstruct
## How to use it
How to use it

Prepare the standard dataset only:

python .\scripts\run_brush.py --prepare-only --clean-input --verbose

Run Brush on the current workspace:

python .\scripts\run_brush.py --dataset-root ".\experiments\indoor_h95\data\brush\input_colmap" --with-viewer --verbose

Run Brush and also open the viewer:

python .\scripts\run_brush.py --clean-input --with-viewer --verbose

Pass extra Brush CLI args after the dataset path:

python .\scripts\run_brush.py --clean-input --brush-args -- --with-viewer

If you want to target a specific experiment workspace:
...
### 3
the current COLMAP stage is still the main place to refine the 360→perspective strategy.

python .\scripts\pipeline.py --preset overlap_6view_h95 --overwrite --clean-convert --clean-prepare --reset-colmap --verbose
python .\scripts\pipeline.py --preset overlap_8view_h85 --overwrite --clean-convert --clean-prepare --reset-colmap --verbose