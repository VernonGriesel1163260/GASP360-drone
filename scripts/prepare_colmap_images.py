...
def ensure_dirs(root: Path, logger) -> dict[str, Path]:
    paths = {
        ...
        "front": root / "data" / "frames_perspective" / "front",
        "right": root / "data" / "frames_perspective" / "right",
        "back": root / "data" / "frames_perspective" / "back",
        "left": root / "data" / "frames_perspective" / "left",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Views to include. Default: front right back left",
    )
    parser.add_argument(
        "--input-prefix",
        type=str,
    ...
    parser.add_argument(
        "--clean",
        action="store_true",
        help="Delete existing files in data/colmap/images before copying.",
    )
    parser.add_argument(
        "--copy-mode",
        choices=["copy", "hardlink"],
        default="copy",
        help="How to place files into COLMAP images folder. Default: copy",
    )
    parser.add_argument(
        "--strict",
        action="store_true",
        help="Fail if any base frame is missing one or more requested views.",
    )
    parser.add_argument(
        "--limit",
        type=int,
        default=None,
        help="Only prepare the first N base frames after grouping.",
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
    ...
    invalid = [v for v in args.views if v not in DEFAULT_VIEWS]
    if invalid:
        parser.error(f"Invalid view(s): {invalid}. Valid views: {DEFAULT_VIEWS}")
    ...
    for view_name in views:
        ...
        for file_path in files:
            ...
            if base_stem is None:
                logger.debug(
                    "Skipping file with unexpected naming pattern: %s",
                    file_path.name,
                )
                continue
...
def validate_groups(
    ...
    for base_stem, view_map in grouped.items():
        ...
        if missing:
            ...
            logger.warning(
                "Base frame '%s' is missing views: %s",
                base_stem,
                missing,
            )
        else:
...
def copy_or_link_file(src: Path, dst: Path, mode: str, logger) -> None:
    if dst.exists():
        dst.unlink()
        logger.debug("Removed existing destination file before replace: %s", dst)
    ...
    if mode == "copy":
        ...
        try:
            ...
            shutil.copy2(src, dst)
            logger.warning(
                "Hardlink failed, fell back to copy: %s -> %s",
                src,
                dst,
            )
    else:
...
def main() -> int:
    ...
    try:
        ...
        logger.info("Views included: %s", args.views)
        logger.info("Preparation completed successfully")

        sample_outputs = sorted(output_dir.glob("*"))
        for sample in sample_outputs[:10]:
            logger.info("Sample output: %s", sample.name)
