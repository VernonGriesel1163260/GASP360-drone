...
def summarize_extraction_metadata(paths: dict[str, Path], logger) -> dict:
    ...
    stream_frame_counts = {
        f"stream_{item.get('stream_index'):02d}": item.get("frame_count")
        for item in selected_streams
        if item.get("stream_index") is not None
    }
    total_frame_count = sum(int(item.get("frame_count") or 0) for item in selected_streams)
    flat_frame_count = int(payload.get("flat_frame_count") or 0)
    ...
    summary.update(
        {
            ...
            "frame_prefix": payload.get("frame_prefix"),
        }
    )
...
def summarize_normalization_metadata(paths: dict[str, Path], logger) -> dict:
    ...
    summary.update(
        {
            ...
            "orientation_signature": payload.get("orientation_signature"),
    ...
    logger.info("Normalization metadata orientation signature: %s", summary["orientation_signature"])
    ...
    if best_json.exists():
        try:
            ...
            if best_model:
                ...
                result["points3D"] = best_model.get("points3D")
                result["observations"] = best_model.get("observations")
        except Exception as exc:
            logger.warning("Failed to parse model inspection JSON: %s", exc)

    elif best_txt.exists():
...
def print_report(summary: dict) -> None:
    ...
    print(f"  orientation_signature={normalization['orientation_signature']}")
    print(f"  transforms={normalization['transforms']}")
    ...
    print(f"COLMAP database: exists={summary['database']['exists']} size_mb={summary['database']['size_mb']:.2f}")
    ...
    print(f"  points3D={best['points3D']}")
    print(f"  observations={best['observations']}")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Summarize pipeline outputs after a run.")
...
def write_workspace_context(paths: dict[str, Path], summary: dict, logger) -> None:
    payload = {
        ...
        "extraction": summary["extraction"],
        "normalization": summary["normalization"],
        "projection": summary["projection_metadata"],
        "metrics": {
            "registered_images": summary["best_model"].get("registered_images"),
            "total_input_images": summary["best_model"].get("total_input_images"),
            "registration_ratio": summary["best_model"].get("registration_ratio"),
            "points3D": summary["best_model"].get("points3D"),
            "observations": summary["best_model"].get("observations"),
        },
        "colmap": {
            ...
            "best_model_name": summary["best_model"].get("selected_model_name"),
            "best_model_path": summary["best_model"].get("selected_model_path"),
        },
    }
